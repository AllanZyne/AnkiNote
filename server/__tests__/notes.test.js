...
async function setup() {
  ...
  const deck = (await request(app).post('/api/decks').send({ name: 'B' })).body;
  return { ntId: nt.id, deckId: deck.id };
}
...
describe('note routes', () => {
  ...
  it('creates a note and lists it in its deck', async () => {
    const created = await request(app).post('/api/notes')
      .send({ noteTypeId: ids.ntId, deckId: ids.deckId, values: { Front: 'hola', Back: 'hi' } });
    expect(created.status).toBe(201);
    const list = await request(app).get(`/api/notes?deck=${ids.deckId}`);
    expect(list.body).toHaveLength(1);
  });

  it('searches notes by content', async () => {
    await request(app).post('/api/notes')
      .send({ noteTypeId: ids.ntId, deckId: ids.deckId, values: { Front: 'gato', Back: 'cat' } });
  ...
  it('updates and deletes a note', async () => {
    const created = await request(app).post('/api/notes')
      .send({ noteTypeId: ids.ntId, deckId: ids.deckId, values: { Front: 'a', Back: 'b' } });
  ...
  it('returns 400 on bad foreign key reference', async () => {
    const res = await request(app).post('/api/notes')
      .send({ noteTypeId: 99999, deckId: ids.deckId, values: { Front: 'a' } });
